...
private:
    ...
    int actionsToday = 0;        // Ñ÷åò÷èê äåéñòâèé çà äåíü
    bool isOverworked = false;   // Ôëàã ïåðåãðóçà
    bool firstActionOfDay;
    static void PrintAnimated(const std::wstring& text, int delayMs = 1);
    std::vector<std::wstring> prevActions; // ïðåäûäóùåå ìåíþ äåéñòâèé
    bool skipUI = false;
    int rentDue = 55000;
    ...
    int taxesCost = 2000;
    bool rentPaid = false;
    static const int MAX_DAY = 10; // îãðàíè÷åíèå íà äåìîíñòðàöèþ
